...
public class RerootService extends AService<Pair> {
	...
	public void setupCalculation(MainWindow window, RerootOrReorderTrees.RootBy rootBy, String[] outgroup) {
		...
		setCallable(() -> {
			...
			return new Pair(originalTrees, rerootedTrees);
		});

		setOnSucceeded(e -> {
			var originals = getValue().originalTrees();
			var rerooted = getValue().rerootedTrees();
...
record Pair(TreesBlock originalTrees, TreesBlock rerootedTrees) {
}
