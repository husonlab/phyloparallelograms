...
/*
 * PhyloFusion.java Copyright (C) 2026 Daniel H. Huson
 *
...
public class PhyloFusion extends Application {
    ...
    protected static void parseArguments(String[] args) throws UsageException {
        var options = new ArgsOptions(args, PhyloFusion.class, "Run the PhyloFusion on a collection of trees");
        ...
        if (showVersion) {
            ...
            System.err.println(jloda.util.Version.getVersion(PhyloFusion.class, ProgramProperties.getProgramName()));
    ...
    public static String getDefaultPropertiesFile() {
        if (ProgramProperties.isMacOS())
            return System.getProperty("user.home") + "/Library/Preferences/Phylofusion.def";
        else
            return System.getProperty("user.home") + File.separator + ".Phylofusion.def";
    }
}
